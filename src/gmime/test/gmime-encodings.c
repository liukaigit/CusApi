...
g_mime_encoding_base64_encode_step (const unsigned char *inbuf, size_t inlen, unsigned char *outbuf, int *state, guint32 *save)
{
	...
	if (inlen + ((unsigned char *)save)[0] > 2) {         //save4个字节，save[0]存储剩余多少个字节，当然最多为2，因为3个字节一组进行编码
		const unsigned char *inend = inbuf + inlen - 2;    //防止inptr越界访问
		register int c1 = 0, c2 = 0, c3 = 0;
		register int already;
		
		already = *state;        //编码每76个字符就换行，存储编码组数(0-19)
...
int code_convert(char *from_charset,char *to_charset,char *inbuf,int inlen,char *outbuf,int outlen){
	...
	cnt = iconv(cd,&inbuf,&inlen,&outbuf,&outlen);
	if((size_t)-1 == cnt){
		iconv_close(cd);
		printf("iconv failed.\n");
		return -1;
...
int main(void){
	...
	//code_convert("gbk","utf-8",decode_outbuf,dlen,outbuf,sizeof(outbuf));

	//printf("dlen = %d,outbuf = %s\n",dlen,outbuf);
